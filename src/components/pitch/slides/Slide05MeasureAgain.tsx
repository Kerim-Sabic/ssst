...
/** phases: 0 flagged · 1 resting+instructions · 2 verifying · 3 confirmed */
export function Slide05MeasureAgain() {
  ...
  useEffect(() => {
    setPhase(0);
    const ts = [
      window.setTimeout(() => setPhase(1), reduce ? 100 : 1700),
      window.setTimeout(() => setPhase(2), reduce ? 200 : 4300),
      window.setTimeout(() => setPhase(3), reduce ? 300 : 5700),
    ];
  ...
  return (
    <Slide>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <Kicker className="mb-4">Signature feature · interactive</Kicker>
          <motion.h1
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1fr] lg:items-stretch">
        ...
        <div className="flex flex-col gap-4">
          <motion.div
            ...
            animate={
              reduce
                ? { opacity: 1, scale: 1 }
                : { opacity: 1, scale: 1, x: [0, -8, 8, -5, 5, 0] }
            }
            transition={{
          ...
          >
            ...
            <p className="tnum mt-1 text-[clamp(44px,7vw,64px)] font-extrabold leading-none tracking-tighter2 text-ink">
              <BPCounter key={`bp1-${run}`} sys={184} dia={118} delay={0.2} duration={0.9} />
              ...
              >
                ...
                <div>
                  ...
                  <p className="tnum mt-1 text-[clamp(36px,5vw,52px)] font-extrabold leading-none tracking-tighter2 text-concern">
                    <BPCounter key={`bp2-${run}`} sys={178} dia={112} delay={0.1} duration={0.9} />
                  </p>
                </div>
        ...
        >
          <div className="flex items-center gap-5">
            <RestRing run={run} active={phase >= 1} />
            <div>
              ...
              <p className="mt-1 text-[13px] leading-snug text-muted">
                A short rest gives a more accurate reading.
              </p>
            </div>
          </div>
          <ul className="mt-5 space-y-2">
            {instructions.map((ins, i) => (
              <motion.li
                ...
                transition={{ delay: phase >= 1 ? i * 0.16 : 0, duration: 0.42, ease }}
      ...
      <div className="mt-6 flex items-center gap-2.5">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {phase === 0 && "Abnormal reading flagged — not escalating yet."}
...
function RestRing({ run, active }: { run: number; active: boolean }) {
  const reduce = useReducedMotion();
  const R = 34;
  const C = 2 * Math.PI * R;
  return (
